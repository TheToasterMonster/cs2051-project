...
(Show model vs actual Ulam numbers) (Maybe also line y = 13.51x)
...
Let's consider an analag to the Twin Prime Conjecture, but for Ulam numbers. The Twin Prime Conjecture states that there are an infinite number of pairs of primes ($p$, $p + 2$) that exist. If we were to bring over this idea to the Ulam sequence, we could theorize that there exists an infinite number of Ulam number pairs $(x, x + 2)$ in the Ulam sequence. Because Ulam numbers can be odd or even, we could change the conjecture to consider pairs $(x, x + 1)$. However, this is not backed heuristically, as there have only been 4 such pairs in the first billion Ulam numbers. So, we will go with the slightly looser conjecture. 

Proving this conjecture for the Ulam numbers perhaps does not have any clear consequences. But the point in this exercise is to show the power, and perhaps the flaws, of Cramer's random modeling technique for these types of sequence. So under the assumption that our random model accurately models the distribution of Ulam numbers, how can one heuristically see that our conjecture is true.

(Apply random model to informally prove our analog twin prime conjecture)

As it stands, applying a random mode l
